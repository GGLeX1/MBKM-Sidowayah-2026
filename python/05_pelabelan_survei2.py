...
ax.set_title("Survei 2 — dari paham menuju melakukan", color=c.GELAP, fontweight="bold")
fig.tight_layout()
fig.savefig(keluaran / "05_tingkat_survei2.png", dpi=150, bbox_inches="tight")
plt.close(fig)
 
kolom = ["id", "Umur", "Pendidikan Terakhir"] + c.fitur_survei2 + ["PK1_niat", "PK2_sering", "PK3_kegiatan", "PK4_rutin", "label", "label_num", "tingkat"]
df[[k for k in kolom if k in df.columns]].to_csv(keluaran / "survei_2_labeled.csv", index=False)
print(f"\nGambar -> output/05_tingkat_survei2.png")
print(f"Data   -> output/survei_2_labeled.csv")